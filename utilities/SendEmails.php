...
//    $message .= '<h3 style="text-align: center;">Server: ' . $_SERVER['REQUEST_URI'] . '</h3>';
    ...
    if(mapreqEnabled)
    {
        ...
        $unit = checkPlural("request", $count);
		$message .= 'Automatic requests: ' . colorizeDangerousValuesHigher($count, $unit, 2000, 2500, '') . '</p>';
    }
...
function getGameCountList()
{
	...
	for($i = 0; $i < $count; $i++)
	{
		if(isset($gameCountList[$gameList[$i]]))
		{
			...
			array_push($output, $gameList[$i] . ': ' . colorizeDangerousValuesLower($gameCountList[$gameList[$i]], $unit, 4, 0, ''));
		}
		else
		{
			array_push($output, $gameList[$i] . ': ' . colorizeDangerousValuesLower(0, 'servers', 1, 0, ''));
		}
	}
